...
(function($) {
    var methods = {
        ...
        get: function(param, string) {
            var object = $.urlParam('getAll', string);
            return object[param.toLowerCase()];
        },
        exists: function(param, string) {
            var value = $.urlParam('get', param.toLowerCase(), string);
            return (typeof value !== 'undefined')
        }
    };
    $.urlParam = function(method) {
        if (methods[method]) {
            ...
            $.error('Method ' + method + ' does not exist on $.urlParam');
            return false;
        }
    };
})(jQuery)